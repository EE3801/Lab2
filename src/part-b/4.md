...
181105_Block1.nev   data_raw6.hkl   RawData_T1-400/
```

We will be using the NEV file, which contains the markers sent by Unity, and the HKL (Hickle) file, which contains the neural signal from one of the 110 channels recorded.
...
```
If you encounter a problem, try quitting ipython and upgrade hickle:

```shell
(aws) $ pip install hickle --upgrade
```
before running ipython and trying the load command again.
