...
```
 ...
 The data is normally stored in EDF files, which are binary files that we will need to use a Python package to read. But since it might take a while to read the files, the files have been converted to PKL (Pickle) files to make them easier to load.
