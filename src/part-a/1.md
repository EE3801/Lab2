...
## 3.
Install GitHub Desktop on your computer [here](https://desktop.github.com).

## 4.
Using GitHub Desktop, clone the forked GitHub repository into a directory on your computer like `Documents/EE3801`.
...
>
> **Avoid** folder name with space (eg. `Documents/EE3801 Lab Work`) unless you know what you are doing and are familiar with how path syntax work in CLI.

You can find instructions on how to clone a repository [here](https://docs.github.com/en/desktop/contributing-and-collaborating-using-github-desktop/cloning-a-repository-from-github-to-github-desktop).

If you are prompted to choose whether you will be contributing to the project or just using the code for your own purposes (see below), select the first choice (i.e. contributing to the project).
...
> 2. Then make sure you install the `Intel x86_64` version of Miniconda and Anaconda *NOT* the `Apple M1` version. This is because some package rely on Cython to compile correctly to run (eg. pyedfread, ...), so the `Apply M1` version wouldn't work (in general however, outside of this course, you should use the `Apple M1` version for best performance).
...
> <p class="note">Note
> 
> Windows users should enter the commands above using the Anaconda Prompt
