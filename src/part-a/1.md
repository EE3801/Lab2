...
> 2. Then make sure you install the `Intel x86_64` version of Miniconda and Anaconda *NOT* the `Apple M1` version. This is because some package rely on Cython to compile correctly to run (eg. pyedfread, ...), so the `Apply M1` version will not work (in general however, outside of this course, you should use the `Apple M1` version for best performance).
